...
def find_config_file(path_to_config_file=None):
    """
    Find the config.yaml file. Priority:
    0. If the path is supplied
    ...
    """
    if os.path.isfile(str(path_to_config_file)):
...
def init_config(local_options_name = "local_configuration", path_to_config_file=None):
    config_path = find_config_file(path_to_config_file)
